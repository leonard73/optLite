...
void free_bmpRes()
{
    usleep(10);
    if(bmpFile_memPtr)
        free(bmpFile_memPtr);bmpFile_memPtr=0;
    if(rawDataPtr)
        free(rawDataPtr);rawDataPtr=0;
}
...
void convert_gray_to_bgr888(unsigned char *  gray,unsigned char * rgb888,unsigned int pixelNb)
{
    for(unsigned int i=0;i<pixelNb;i++)
    {
        unsigned char gray_v = *gray++;
        *rgb888++            = gray_v;
        *rgb888++            = gray_v;
        *rgb888++            = gray_v;
    }
...
{
    unsigned  char * rgb = (unsigned char*)malloc(width*height*sizeof(unsigned char)*3);
    convert_gray_to_bgr888(p_dataGray8,rgb,width*height);
    save_RawRGB_bmpFile(savePath, rgb, width*height*3, width, height);
    free(rgb);
}
